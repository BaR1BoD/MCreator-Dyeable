...
<#compress>
public class ${name}Item extends Item <#if data.dyeableItem.isDyeable()>implements IDyeableItem</#if> {
	...
	<#if data.dyeableItem.isDyeable()>
	@Override public int getDefaultColor() {
	  return ${data.dyeableItem.defaultColor().getRGB()};
	}
	</#if>
