...
@Mod("${modid}") public class ${JavaModName} {
    ...
    <#if w.getGElementsOfType("dyeable_item")?filter(e -> e.dyeableItem.isDyeable())?size != 0>
		modEventBus.addListener(RegisterColor::onItemColorRegister);
		</#if>
		...
		<#if w.getGElementsOfType("dyeable_item")?filter(e -> e.dyeableItem.isDyeable())?size != 0>${JavaModName}RecipeSerializers.REGISTRY.register(modEventBus);</#if>
